                  ...
                  <div id="product-5389" class="product type-product post-5389 status-publish first instock product_cat-sides product_tag-delivery has-post-thumbnail shipping-taxable purchasable product-type-simple">
                     ...
                     <div class="woocommerce-product-gallery woocommerce-product-gallery--with-images woocommerce-product-gallery--columns-4 images" data-columns="4" style="opacity: 1; transition: opacity 0.25s ease-in-out 0s;">
                        <div class="woocommerce-product-gallery__wrapper">
                           <div data-thumb="{{ $detail->image }}" data-thumb-alt="" class="woocommerce-product-gallery__image"><a href="{{ $detail->image }}" data-rel="woo-product-gallery"><img width="600" height="600" src="{{ $detail->image }}" class="wp-post-image" alt="" decoding="async" title="{{ $detail->title }}" data-caption="" data-src="{{ $detail->image }}" data-large_image="{{ $detail->image }}" data-large_image_width="1440" data-large_image_height="1440" loading="lazy"></a></div>
                        </div>
                     </div>

                     <div class="summary entry-summary">
                        <h1 class="product_title entry-title">{{ $detail->title }}</h1>
                        <p class="price"><span class="woocommerce-Price-amount amount"><bdi>{{ $detail->gia }} <?php if($detail->donvitien=='Dollar') echo '$'; else echo $detail->donvitien ?></bdi></span></p>
                        <div class="woocommerce-product-details__short-description">
                           <p>{{ $detail->mota }}</p>
                        </div>


                        <form class="cart" action="{{ route('frontend.order.add_to_cart') }}" method="get" enctype="multipart/form-data">
                           @csrf
                           <div class="wapf">
                              ...
                              <div class="wapf-product-totals" data-product-type="simple" data-product-price="17.45" data-product-id="5389" style="display: block;">
                                 <div class="wapf--inner">
                                    <div>
                                       ...
                                       <span id="product-total" class="wapf-product-total price amount">{{ number_format($detail->gia) }} <?php if($detail->donvitien=='Dollar') echo '$'; else echo $detail->donvitien ?></span>
                                    </div>
                                    <!-- <div>
                        ...
                        @foreach($posts as $item)
                           <li class="product type-product post-5385 status-publish instock product_cat-main product_tag-delivery has-post-thumbnail shipping-taxable purchasable product-type-simple">
                              <a href="/chi-tiet-sp/{{ $item->alias }}.html" class="woocommerce-LoopProduct-link woocommerce-loop-product__link"><img width="300" height="300" src="{{ $item->image }}" class="attachment-woocommerce_thumbnail size-woocommerce_thumbnail" alt="" decoding="async" loading="lazy">
                                 ...
                                 <span class="price"><span class="woocommerce-Price-amount amount"><bdi>{{ $item->gia }} <?php if($item->donvitien=='Dollar') echo '$'; else echo $item->donvitien ?></bdi></span></span>
                              </a><a href="/chi-tiet-sp/{{ $item->alias }}.html" data-quantity="1" class="button product_type_simple add_to_cart_button" data-product_id="5385" data-product_sku="" aria-label="Add “Tuna Roast Source” to your cart" aria-describedby="" rel="nofollow">Select options</a>
                           </li>
                           @endforeach
                        </ul>
